...
The outcome is that the main payload is not executed if Sleep() is shortcutted by hooks. Sleep() is hooked by default in many sandboxes as an attempt to trick malware families that purposelly implements delays so automated analyses would timeout. 
