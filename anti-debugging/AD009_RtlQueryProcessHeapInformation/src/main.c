...
bool __is_debugged() {
    ...
    if (!SUCCEEDED(RtlQueryProcessHeapInformation(pDebugBuffer)))
        return false;
...
int main() {
    if(__is_debugged())
        MessageBoxA(NULL, "[+] The process is in Debug mode.", "Anti-debug 016", MB_OK);
    else
        MessageBoxA(NULL, "[+] The process is NOT in Debug mode.", "Anti-debug 016", MB_OK);
