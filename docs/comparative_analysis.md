...
1. **This project** (`anti`) - 72 techniques
...
| **Anti-Debugging** | 59 techniques | ~34 techniques | 61 techniques |
...
| Heap Protection (sentinel patterns) | AD051 | - | Yes |
...
| Stack Segment Register (push ss/pop ss) | AD053 | - | Yes |
...
| Detecting function patches | AD049 | - | Yes |
...
| Process jobs | AD058 | Yes | - |
| API hook detection | AD049 | Yes | - |
| EnumWindows + SuspendThread | AD054 | - | Yes |
| WudfIsAnyDebuggerPresent | AD059 | Yes | - |
| DbgSetDebugFilterState | AD055 | - | Yes |
| Selectors | - | - | Yes |
...
| AD058 | Process Job | Job object enumeration for debugger detection |
...
| ~~Stack Segment Register~~ | ~~Assembly~~ | ~~Low~~ - Implemented as **AD053** |
...
| Selectors | Misc | Low |
| ~~DbgSetDebugFilterState~~ | ~~Misc~~ | ~~Low~~ - Implemented as **AD055** |
...
- Limited anti-sandbox coverage (12 vs Al-Khaser's 90+)
...
| -------- | :-----------: | :---------: | :-----------: |
| Total anti-debug techniques | 59 | ~34 | 61 |
| Shared with Check Point | 51 / 61 (84%) | ~25 / 61 (41%) | - |
| Shared with Al-Khaser (anti-debug) | 31 / 34 (91%) | - | ~25 / 34 (74%) |
...
- ~~CreateFile (exclusive open)~~ - Implemented as **AD046**
- ~~NtQuerySystemInformation (KernelDebugger)~~ - Implemented as **AD047**

### Medium-Priority Additions (present in one reference)

1. **CPUID hypervisor detection** - Essential anti-VM technique (Al-Khaser)
3. **WMI-based VM queries** - Modern anti-VM standard (Al-Khaser)
4. **Anti-disassembly techniques** - New category, 6 techniques from Al-Khaser
